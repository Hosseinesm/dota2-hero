...
const App: React.FC = () => {
  ...
  const loadHeroes = useCallback(async () => {
    ...
    try {
      ...
      // Keep the original error message if it's custom, otherwise generic
      const msg = err.message || "مشکل در برقراری ارتباط با سرور. لطفا اتصال اینترنت خود را بررسی کنید.";
      setError(msg);
    } finally {
      setLoading(false);
    }
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[50vh] gap-6 text-center px-4">
            ...
            <div className="max-w-md">
              ...
              <p className="text-red-300 bg-red-950/50 p-2 rounded border border-red-900/50 mb-4">{error}</p>
              
              <div className="text-sm text-slate-400 text-right bg-slate-900 p-4 rounded-lg border border-slate-800">
                <p className="font-bold text-slate-300 mb-2">راهنمای رفع مشکل:</p>
                <ul className="list-disc list-inside space-y-1">
                  <li>اگر از ایران متصل هستید، حتماً <strong>VPN</strong> خود را روشن کنید.</li>
                  <li>اگر روی گوشی خطای "Permission" می‌گیرید، احتمالاً لینک خصوصی است. باید از لینک عمومی (Deploy شده) استفاده کنید.</li>
                </ul>
              </div>
            </div>
            <button 
